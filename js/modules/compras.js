        ...
        <div class="table-responsive">
          <table class="table mb-0">
            <thead><tr>
              ...
              <th>Data</th><th>Valor</th><th>Obs.</th><th class="text-end pe-3">Ações</th>
            </tr></thead>
            <tbody>
            ${dados.length ? dados.map(r => `<tr>
              ...
              <td class="fw-bold text-primary">${fmtMoney(r.valor)}</td>
              <td><small class="text-muted">${escHtml((r.observacao||'').slice(0,30))}</small></td>
...
function formCompras(r={}) {
  ...
  `<div class="row g-3">
    ...
    <div class="col-md-6"><label class="form-label fw-semibold">VALOR (R$) *</label>
      <div class="input-group"><span class="input-group-text">R$</span>
        <input type="number" class="form-control" id="cValor" step="0.01" min="0" value="${r.valor||''}"></div></div>
    <div class="col-12"><label class="form-label">OBSERVAÇÃO</label>
      <textarea class="form-control" id="cObs" rows="2" placeholder="Informações adicionais...">${escHtml(r.observacao||'')}</textarea></div>
  </div>`,
...
async function salvarCompras(id) {
  ...
  if (!valor)     { toast('Valor obrigatório','danger'); return; }
  const obj = {
    descricao, valor,
  ...
  try {
    if (id) { await update('compras',id,obj); toast('Compra atualizada!'); }
    else    { await insert('compras',obj); toast('Compra registrada!'); }
