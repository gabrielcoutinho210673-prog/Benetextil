        ...
        <div class="table-responsive">
          <table class="table mb-0">
            ...
            ${dados.length ? dados.map(p => {
              const alerta = (parseFloat(p.estoque)||0) <= (parseFloat(p.estoque_minimo)||0);
              return `<tr class="${alerta?'table-warning':''}">
                ...
                <td class="fw-bold ${alerta?'text-danger':''}">${parseFloat(p.estoque)||0}</td>
...
function formProduto(p={}) {
  ...
  `<div class="row g-3">
    ...
    <div class="col-md-3"><label class="form-label fw-semibold">ESTOQUE ATUAL</label>
      <input type="number" class="form-control" id="pEstoqueAtual" step="0.01" min="0" value="${p.estoque||0}"></div>
    ...
    <div class="col-md-3"><label class="form-label">CUSTO (R$)</label>
      <div class="input-group"><span class="input-group-text">R$</span>
        <input type="number" class="form-control" id="pCusto" step="0.01" min="0" value="${p.preco_custo||''}"></div></div>
    ...
    <div class="col-12"><label class="form-label">OBSERVAÇÃO</label>
      <textarea class="form-control" id="pObs" rows="2">${escHtml(p.observacoes||'')}</textarea></div>
  </div>`,
...
async function salvarProduto(id) {
  ...
  const obj = {
    ...
    estoque:        parseFloat(document.getElementById('pEstoqueAtual').value)||0,
